...
func HandleSaveBook(w http.ResponseWriter, r *http.Request) {
	...
	if id == 0 {
		id, err = insertBook(name, author, pages, publicationDate)
	} else {
		id, err = updateBook(id, name, author, pages, publicationDate)
	}
...
func HandleListBooks(w http.ResponseWriter, r *http.Request) {
	...
	buf, err := ioutil.ReadFile("view/index.html")
	if err != nil {
		renderErrorPage(w, err)
		return
	}
...
func HandleViewBook(w http.ResponseWriter, r *http.Request) {
	...
	buf, err := ioutil.ReadFile("view/book.html")
	if err != nil {
		renderErrorPage(w, err)
		return
	}
...
func renderErrorPage(w http.ResponseWriter, errorMsg error) {
	buf, err := ioutil.ReadFile("view/error.html")
	if err != nil {
		log.Printf("%v\n", err)
		fmt.Fprintf(w, "%v\n", err)
		return
